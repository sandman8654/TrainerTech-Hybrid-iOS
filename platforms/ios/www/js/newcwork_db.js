		...
		tx.executeSql(sqlite, [], function(tx, results) {
			...
			setTimeout(function(){
				if(workout){
			        ...
			        sqlite = 'SELECT `c`.`id`, `c`.`name`, `c`.`workout_id`, `en`.`notes` as status FROM (`circuit` as c) LEFT JOIN `exercise` as e ON `e`.`circuit_id` = `c`.`id` LEFT JOIN `exercise_notes` as en ON `en`.`exercise_id` = `e`.`id` WHERE `c`.`workout_id` = "'+workout.id+'" GROUP BY `c`.`id` ORDER BY `c`.`id` asc';
